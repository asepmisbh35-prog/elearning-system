{{-- resources/views/guru/quizzes/questions/edit.blade.php --}}
@extends('layouts.app')
...
<div class="max-w-2xl mx-auto px-4 md:px-6 py-6 md:py-8" x-data="{ saving: false }">
    ...
    <div class="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br from-[#4F46E5] via-[#4338CA] to-[#3730A3] p-5 md:p-8 text-white mb-6 rise-in">
        ...
        <h1 class="relative text-xl md:text-2xl font-bold mt-2">Edit Soal</h1>
        <p class="relative text-sm text-indigo-100 mt-1">Perbarui detail soal dan konten sesuai tipe interaksi.</p>
    </div>
    ...
    @php
        ...
        $optionTexts    = $question->options->pluck('option_text')->toArray();
        $sortingTexts   = $question->options->pluck('option_text')->toArray();
        $oldBlanks      = old('blank_keywords', collect($question->meta['blanks'] ?? [])->map(fn($kw) => implode(', ', $kw))->toArray());
        $oldPairsLeft   = old('pairs_left', collect($question->meta['pairs'] ?? [])->pluck('left')->toArray());
        $oldPairsRight  = old('pairs_right', collect($question->meta['pairs'] ?? [])->pluck('right')->toArray());
    @endphp

    <form method="POST" action="{{ route('guru.quizzes.questions.update', $question) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <select name="type" id="typeSelect" onchange="toggleTypeSections()"
                ...
                <option value="true_false" @selected(old('type', $question->type) === 'true_false')>Benar/Salah</option>
                <option value="short_answer" @selected(old('type', $question->type) === 'short_answer')>Isian Singkat</option>
                <option value="true_false_swipe" @selected(old('type', $question->type) === 'true_false_swipe')>True/False Swipe</option>
                <option value="sorting" @selected(old('type', $question->type) === 'sorting')>Sorting/Urutan</option>
                <option value="fill_blank" @selected(old('type', $question->type) === 'fill_blank')>Fill in the Blank</option>
                <option value="timer_challenge" @selected(old('type', $question->type) === 'timer_challenge')>Timer Challenge</option>
                <option value="drag_drop" @selected(old('type', $question->type) === 'drag_drop')>Drag & Drop (Mencocokkan)</option>
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="question_text" id="questionText" rows="3"
                      class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-[#4F46E5] focus:border-transparent transition-colors duration-150" required>{{ old('question_text', $question->question_text) }}</textarea>
        </div>

        @if ($question->image_path)
            <div>
                <img src="{{ Storage::url($question->image_path) }}" class="h-24 rounded-xl border border-gray-200 mb-2">
            </div>
        @endif
        <div>
        ...
        <div>
            ...
            <input type="number" name="points" value="{{ old('points', $question->points) }}" min="1" max="100"
        ...
        <div id="section-multiple_choice" class="border border-gray-200 rounded-xl p-4">
            ...
            <div id="optionsList" class="space-y-2">
                @php $oldOptions = old('options', in_array($question->type, ['multiple_choice', 'timer_challenge']) && count($optionTexts) ? $optionTexts : ['', '']); @endphp
                @foreach ($oldOptions as $i => $val)
                    <div class="flex items-center gap-2 option-row">
                        <input type="radio" name="correct_option" value="{{ $i }}"
                               {{ old('correct_option', $correctIndex !== false ? $correctIndex : 0) == $i ? 'checked' : '' }}
                               class="text-[#4F46E5] focus:ring-[#4F46E5]">
        ...
        <div id="section-true_false" class="border border-gray-200 rounded-xl p-4" style="display:none;">
            ...
            <div class="flex gap-4">
                @php
                    $tfOld = old('true_false_answer', optional($question->options->firstWhere('option_text', 'Benar'))->is_correct ? 'true'
                              : (optional($question->options->firstWhere('option_text', 'Salah'))->is_correct ? 'false' : null));
                @endphp
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="true_false_answer" value="true" {{ $tfOld === 'true' ? 'checked' : '' }} class="text-[#4F46E5] focus:ring-[#4F46E5]"> <span class="text-sm">Benar</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="true_false_answer" value="false" {{ $tfOld === 'false' ? 'checked' : '' }} class="text-[#4F46E5] focus:ring-[#4F46E5]"> <span class="text-sm">Salah</span>
                </label>
            </div>
        </div>
        ...
        <div id="section-short_answer" class="border border-gray-200 rounded-xl p-4" style="display:none;">
            ...
            <input type="text" name="answer_keywords"
                   value="{{ old('answer_keywords', is_array($question->answer_keywords) ? implode(', ', $question->answer_keywords) : '') }}"
        ...
        <div id="section-sorting" class="border border-gray-200 rounded-xl p-4" style="display:none;">
            ...
            <div id="sortingList" class="space-y-2">
                @php $oldSorting = old('sorting_items', $question->type === 'sorting' && count($sortingTexts) ? $sortingTexts : ['', '']); @endphp
        ...
        <div id="section-timer_challenge" class="border border-gray-200 rounded-xl p-4" style="display:none;">
            ...
            <input type="number" name="time_limit_seconds"
                   value="{{ old('time_limit_seconds', $question->meta['time_limit_seconds'] ?? 15) }}" min="5" max="300"
            ...
            <input type="number" name="bonus_max_percent"
                   value="{{ old('bonus_max_percent', $question->meta['bonus_max_percent'] ?? 50) }}" min="0" max="100"
        ...
        <div id="section-drag_drop" class="border border-gray-200 rounded-xl p-4" style="display:none;">
            ...
            <div id="pairsList" class="space-y-2">
                @php
                    $pairLeftRows = count($oldPairsLeft) ? $oldPairsLeft : ['', ''];
                @endphp
                @foreach ($pairLeftRows as $i => $val)
                    <div class="flex items-center gap-2 pair-row">
                        ...
                        <input type="text" name="pairs_right[{{ $i }}]" value="{{ $oldPairsRight[$i] ?? '' }}"
...
<script>
    // Nilai blank_keywords lama dari server (hasil edit gagal validasi ATAU dari meta tersimpan)
    const initialBlanks = @json($oldBlanks);
    ...
    function toggleTypeSections() {
        ...
        // Timer Challenge: section terpisah, tapi TETAP tampilkan section pilihan ganda juga (numpuk, sama seperti create.blade.php)
    ...
    function updateBlankRows() {
        ...
        for (let i = 0; i < count; i++) {
            const prefill = existingValues[i] !== undefined ? existingValues[i] : (initialBlanks[i] || '');
            const row = document.createElement('div');
            row.className = 'flex items-center gap-2 mb-2';
            row.innerHTML = `
                <span class="text-xs text-gray-400 w-16 shrink-0">Blank ${i + 1}</span>
                <input type="text" name="blank_keywords[${i}]" class="blank-keyword-input flex-1 border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#4F46E5] transition-colors duration-150" placeholder="Jawaban benar, pisah koma" value="${prefill}">
            `;
    ...
    // Jalankan saat halaman dimuat, termasuk saat redirect balik karena validasi gagal
