{{-- resources/views/siswa/calendar/index.blade.php --}}
@extends('layouts.app')
@section('title', 'Kalender')

@php
    $allEvents   = collect($weeks)->flatten(1)->flatMap(fn ($d) => $d['events']);
    $tugasCount  = $allEvents->where('type', 'tugas')->count();
    $kuisCount   = $allEvents->where('type', 'kuis')->count();
    $rapatCount  = $allEvents->where('type', 'pertemuan')->count();
...
<div class="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-8"
    ...
    <div class="relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br from-[#4F46E5] via-[#4338CA] to-[#3730A3] p-5 sm:p-6 md:p-8 text-white mb-5 md:mb-8">
        ...
        <div class="relative flex items-start justify-between flex-wrap gap-4">
            <div>
                <p class="text-indigo-200 text-[11px] md:text-sm font-medium mb-1 tracking-wide uppercase">Jadwal &amp; Aktivitas</p>
                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold flex items-center gap-2">
                    <span>🗓️</span> Kalender Belajar
                </h1>
                <p class="text-indigo-100 text-xs sm:text-sm mt-1 max-w-sm">
                    Pantau tugas, kuis, dan pertemuan kelasmu bulan ini di satu tempat.
                </p>
            </div>

            <div class="flex items-center gap-1.5 sm:gap-2">
                <a href="{{ route('siswa.calendar.index', ['month' => $start->copy()->subMonth()->month, 'year' => $start->copy()->subMonth()->year]) }}"
                ...
                <a href="{{ route('siswa.calendar.index', ['month' => $start->copy()->addMonth()->month, 'year' => $start->copy()->addMonth()->year]) }}"
                   class="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex items-center justify-center rounded-full bg-white/15 hover:bg-white/25 active:scale-95 transition text-white text-sm">›</a>
            </div>
        </div>

        {{-- Ringkasan bulan ini + filter interaktif --}}
        <div class="relative flex flex-wrap gap-2 mt-5 md:mt-6">
            ...
            <button type="button" @click="toggleFilter('tugas')"
                ...
                <span class="w-2 h-2 rounded-full bg-amber-400"></span> Tugas · {{ $tugasCount }}
            </button>
            <button type="button" @click="toggleFilter('kuis')"
                    ...
                    class="flex items-center gap-1.5 text-[11px] sm:text-xs md:text-sm font-medium px-3 py-1.5 rounded-full transition">
                <span class="w-2 h-2 rounded-full bg-violet-400"></span> Kuis · {{ $kuisCount }}
            </button>
            <button type="button" @click="toggleFilter('pertemuan')"
                    ...
                    class="flex items-center gap-1.5 text-[11px] sm:text-xs md:text-sm font-medium px-3 py-1.5 rounded-full transition">
                <span class="w-2 h-2 rounded-full bg-sky-400"></span> Pertemuan · {{ $rapatCount }}
            </button>
        </div>
    </div>
    ...
    @if ($totalCount === 0)
        <div class="flex flex-col items-center text-center mt-8 md:mt-10">
            ...
            <p class="text-sm text-gray-400">Belum ada tugas, kuis, atau pertemuan bulan ini. Yeay, waktunya santai!</p>
        </div>
    @endif
